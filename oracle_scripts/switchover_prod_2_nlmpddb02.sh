...
# v1.0.2  2021-09-13 Reyaz  Alibux, added code to copy tnsnames.ora for uitwijk
# v1.0.3
################################################################################
# Oracle Settings
export TMP=/tmp
...
echo "copy tnsnames.ora file" >> ${LOG_FILE}
/bin/cp $ORACLE_HOME/network/admin/tnsnames.ora $ORACLE_HOME/network/admin/tnsnames.ora.org >> ${LOG_FILE}
/bin/cp $ORACLE_HOME/network/admin/tnsnames.ora_uitwijk $ORACLE_HOME/network/admin/tnsnames.ora >> ${LOG_FILE}
...
echo >> ${LOG_FILE}
echo "configure rman settings" >> ${LOG_FILE}
echo >> ${LOG_FILE}
rman<<EOF6 >> ${LOG_FILE}
connect target /
connect catalog rman_mpd_PROD_DR/07Woee77@RMANNLP12c
@/home/oracle/scripts/uitwijk_prod_3
EOF6
