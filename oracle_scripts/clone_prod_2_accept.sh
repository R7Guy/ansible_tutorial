#!/bin/bash
###############################################################################
# Program       : clone_accept.sh
# Description   : Creates db clone from prod to accept database
# Author        : Reyaz Alibux
# Date          : 2020-06-22
################################################################################
...
export ORACLE_SID=accept
export LOG_DIR=/home/oracle/log
LOG_FILE="${LOG_DIR}/CLONE_ACCEPT_$(date +%Y_%m_%d).log"
export TARGET_DB=prod
#LIST="reyaz.alibux@cgi.com j.schmeltz@cgi.com john.timmer@cgi.com peter.verschoor@cgi.com javid.sharifi@cgi.com cihan.ceylan@cgi.com ron.vanden.bulk@cgi.com nandita.bhunia@cgi.com sumathi.vunnam@cgi.com"
...
cd /tmp && rm -rf /u02/oradata/ACCEPT/datafile/*.dbf
cd /tmp && rm -rf /u02/oradata/ACCEPT/controlfile/*.ctl
cd /tmp && rm -rf /u03/oradata/ACCEPT/controlfile/*.ctl
cd /tmp && rm -rf /u02/oradata/ACCEPT/onlinelog/*.log
cd /tmp && rm -rf /u03/oradata/ACCEPT/onlinelog/*.log
cd /tmp && rm -rf /u02/oradata/ACCEPT/datafile/*.tmp
...
mkdir -p /u01/app/oracle/admin/accept/adump
mkdir -p /u02/oradata/ACCEPT/controlfile
mkdir -p /u03/oradata/ACCEPT/controlfile
...
connect auxiliary sys/jsuD190gU62jW9@accept
@/home/oracle/scripts/clone_accept
EOF4
...
#perform post_clone_accept script
echo "post_clone_accept" >> ${LOG_FILE}
echo >> ${LOG_FILE}
sqlplus '/as sysdba'<<EOF6 >> ${LOG_FILE}
@/home/oracle/scripts/post_clone_accept.sql
@/home/oracle/scripts/ACCEPT_Directories.sql
@/home/oracle/scripts/purge_unified_audit_trail.sql
exit
EOF6

mailx -s "DB_Clone_MPD_ACCEPT" -a ${LOG_FILE} ${LIST} < /home/oracle/bin/clone_accept.txt
mailx -s "DB_Clone_MPD_ACCEPT" -a ${LOG_FILE} ron.vanden.bulk@cgi.com < /home/oracle/bin/clone_accept.txt
mailx -s "DB_Clone_MPD_ACCEPT" -a ${LOG_FILE} nandita.bhunia@cgi.com < /home/oracle/bin/clone_accept.txt
mailx -s "DB_Clone_MPD_ACCEPT" -a ${LOG_FILE} sumathi.vunnam@cgi.com < /home/oracle/bin/clone_accept.txt
